...
**重要: テスト内容はPdM（非エンジニア）が読んで理解できるように書く。**

- 条件・ユーザー操作・期待される挙動は、実装の内部用語ではなく**プロダクト上の振る舞い**で表現する。
- 以下のような技術用語は避け、平易な言葉に言い換える:
  - `isDirty` → 「入力内容が変更されている状態」
  - `resetField` / `setValue` / `useState` などの実装API名 → 使わない（ユーザーから見た挙動で書く）
  - HTTPステータスコード（`204` 等）、`popstate` / `beforeunload` 等のイベント名 → 「保存の正常完了」「ブラウザの戻る操作」「再読み込み」などに言い換える
  - 「DLG」などの略語 → 「確認ダイアログ」のように正式名称で書く
- 略語・英略記は使わず、画面に表示される文言やユーザー操作の名称をそのまま使う。
