...
- `--base <branch>`: マージ先ブランチ（未指定時はリポジトリのデフォルトブランチを自動検出）
...
- `--base` オプション（指定がなければ Step 1-2 でデフォルトブランチを自動検出）
- `--title` オプション（指定がなければ後でStep 3で自動生成）
- `--ticket` オプション（指定がなければ後でStep 4で対話補完）
- `--no-draft` フラグ（指定がなければドラフトPRとして作成）
- その他のオプション・指示

### Step 1-2: デフォルトブランチの検出（--base 未指定時）

`--base` が指定されていない場合、リポジトリのデフォルトブランチを検出する:

```bash
git remote show origin | grep 'HEAD branch' | sed 's/.*: //'
```

検出したブランチを `{{base}}` として使用する。
...
PRを作成する前に、`/code-review` を実行してブランチの差分をレビューする。

#### 6-1: レビューの実行

`/code-review` スキルを実行し、現在の差分（`git diff origin/{{base}}...HEAD`）に対する正確性バグのレビューを行う。

#### 6-2: レビュー結果の提示と判定

`/code-review` の結果を提示し、指摘の有無に応じて分岐する:

- **指摘なし**: そのまま Step 7 へ進む
- **指摘あり**: 指摘内容を提示し、修正してから進めるか・このまま進めるかをユーザーに確認する
